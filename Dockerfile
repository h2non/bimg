...
RUN DEBIAN_FRONTEND=noninteractive apt-get update && \
  ...
  automake build-essential pkg-config gcc git libc6-dev make \
  gobject-introspection libexpat1-dev libglib2.0-dev \
  libjpeg-turbo8-dev libpng12-dev libwebp-dev libtiff5-dev libgif-dev librsvg2-dev libopenslide-dev libgsf-1-dev \
  libexif-dev libxml2-dev libpoppler-glib-dev libmagickwand-dev libpango1.0-dev libmatio-dev \
  libcfitsio-dev fftw3-dev liborc-0.4-dev

# Build libvips (@see https://libvips.github.io/libvips/install.html)
RUN cd /tmp && \
...
# Compile the binary, to verify compile-time correctness. The Docker build should fail if this step fails.
